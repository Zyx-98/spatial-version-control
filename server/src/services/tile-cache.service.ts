...
export class TileCacheService {
  ...
  private async generateAndCacheTile(
    ...
    this.logger.log(`Generating tile: ${cacheKey}`);
    ...
    try {
      ...
      this.logger.log(
        `Cached tile: ${cacheKey} (generated in ${generateDuration}ms, TTL: ${ttl}s)`,
      );
    } catch (error) {
  ...
  async getDiffTile(
    ...
    if (cachedTile) {
      this.logger.log(
        `Cache HIT (diff): ${cacheKey} (${Date.now() - startTime}ms)`,
      );
      return cachedTile;
    }

    this.logger.log(`Cache MISS (diff): ${cacheKey}`);
  ...
  private async generateAndCacheDiffTile(
    ...
    try {
      const ttl = Math.min(this.getCacheTTL(z), 300); // Max 5 minutes
      await this.cacheManager.set(cacheKey, tile, ttl * 1000);

      this.logger.log(
        `Cached diff tile: ${cacheKey} (generated in ${generateDuration}ms, TTL: ${ttl}s)`,
      );
    } catch (error) {
      this.logger.error(
  ...
  private getCacheTTL(zoom: number): number {
    if (zoom < 6) {
      return 86400; // 24 hours for low zoom (overview)
    } else if (zoom < 9) {
      return 3600; // 1 hour for medium zoom
    } else if (zoom < 12) {
      return 1800; // 30 minutes for high zoom
    } else {
      return 300; // 5 minutes for very high zoom (detailed)
    }
  }
