...
export class BranchService {
  ...
  async detectConflicts(
    ...
    const query = `
      WITH source_features AS (
        -- Get latest features from source branch (using recursive CTE)
      ...
      target_features AS (
        -- Get latest features from target branch
        WITH RECURSIVE commit_chain AS (
          SELECT id, parent_commit_id, created_at, 0 as depth
          FROM commits
          WHERE id = $2
      ...
      -- Find conflicts using PostGIS spatial comparison
      SELECT
    ...
    // For conflicting features, fetch full details (only for conflicts, not all features)
  ...
  async getLatestFeatures(
    ...
    // Optimized: Use single recursive CTE query with optional pagination and bbox filtering
    ...
    const query = `
      WITH RECURSIVE commit_chain AS (
        ...
        -- Recursively follow parent commits
        SELECT c.id, c.parent_commit_id, c.created_at, cc.depth + 1
        FROM commits c
        INNER JOIN commit_chain cc ON c.id = cc.parent_commit_id
        WHERE cc.depth < 1000  -- Safety limit
      ),
      -- Get all features from commit chain with their commit order
      features_with_order AS (
        SELECT
          sf.id,
          sf.feature_id,
      ...
      -- Get only the most recent version of each feature
      latest_features AS (
        SELECT
          id,
        ...
        WHERE rn = 1
          AND operation != 'delete'  -- Exclude deleted features
      )
  ...
  private async getCommitHistory(headCommitId: string): Promise<Commit[]> {
    // Optimized: Use recursive CTE instead of N+1 queries
    const query = `
      WITH RECURSIVE commit_chain AS (
